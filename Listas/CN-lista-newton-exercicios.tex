...
\item (Conjugação de métodos) Quando não se tem um bom ponto de
  ...
  \begin{enumerate}
  \item Qual é a derivada desta função?
  \item Verifique que a função troca de sinais no intervalo
    $[-1,0.1]$, e portanto o método da Bissecção pode ser aplicada
    para encontrar uma raiz aproximada para ela.
  \item Aplique o Método da Bissecção por duas iterações, e encontre
    $x_2$ no intervalo $[-1,0.1]$. Verifique que para este método,
    você precisaria de 12 iterações para atingir a precisão de $0.001$
  \item Use o valor do item anterior como valor inicial $x_0$ do
    Método de Newton, e encontre uma aproximação com precisão de
    $0.001$. (Obs: se você utilizar o critério de parada $\varepsilon =
    |f(x_n)|$, você precisará de 2 iterações. Se você utilizar o
    critério $\varepsilon = |x_n - x_{n-1}|$ precisará de 3 iterações.)
  \end{enumerate}
